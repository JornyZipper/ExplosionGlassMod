...
public class GlassSoundBlocker {
    ...
    public static class CustomSoundType extends SoundType {
        ...
        public CustomSoundType(float volume, float pitch, SoundEvent[] breakSounds, SoundEvent[] stepSounds, SoundEvent[] placeSounds) {
            super(volume, pitch,
                    breakSounds != null && breakSounds.length > 0 ? breakSounds[0] : net.minecraft.sounds.SoundEvents.GLASS_BREAK,
                    stepSounds != null && stepSounds.length > 0 ? stepSounds[0] : net.minecraft.sounds.SoundEvents.GLASS_STEP,
                    placeSounds != null && placeSounds.length > 0 ? placeSounds[0] : net.minecraft.sounds.SoundEvents.GLASS_PLACE,
        ...
        private SoundEvent random(SoundEvent[] sounds) {
            ...
            return sounds[new java.util.Random().nextInt(sounds.length)];
        }
    }
    ...
    public static void setupCustomGlassSounds() {
        try {
            ...
            String[] possibleFieldNames = {"soundType"};
            ...
            soundTypeField.set(Blocks.WHITE_STAINED_GLASS, glassSoundType);
            soundTypeField.set(Blocks.GLASS_PANE, glassSoundType);
            soundTypeField.set(Blocks.WHITE_STAINED_GLASS_PANE, glassSoundType);

            // Применяем к льду
            soundTypeField.set(Blocks.ICE, iceSoundType);
            soundTypeField.set(Blocks.PACKED_ICE, iceSoundType);
            soundTypeField.set(Blocks.BLUE_ICE, iceSoundType);

            System.out.println("[ExplosionGlass] Glass & ice sounds applied successfully!");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
